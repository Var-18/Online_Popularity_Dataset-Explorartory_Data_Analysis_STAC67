...
Step 1: EDA(DV,Plots, slicing, etc), Cleaning Data, and Checking for Co
Linearity Step 2: Step-wise Selection(Lec 20) Prof said it is better
Step 3: Model from Step 2 use all possible subset regressions(Slide 23)
Step 4: Use Model from Step 2(Main Effect Model) and use the colinearity
results as a guide for making an interaction model. Step 5: : Model from
Step 4 use all possible subset regression or another method or tests to
remove or keep interactions to get an Improved Interaction Model. Step
6: Diagnostics on Interaction and Regular  to which ever is better? Use WLS,
Box-cox or whatever if any assumption is violated. Gauss Markov Theorem
Assumptions (LNIE) Step 7: Final Model using whichever transformation
was good,(try and use some transformation from out of the course
syllabus as well ask in OH) Step 8: Repeat Model Diagnostics, Lec 20
onwards, make any changes if necessary(Transform again?) Step 9:
Validation
...
After seeing the data, the first column Url is kind of like an id for
the whole dataset so we keep that, timedelta is non-predictive so we
drop that, n_non_stop_words is all 1 so we drop that, for data channel
we can combine those into one column that specify what channel it is
using pivot_longer(), also for weekday or weekend we can just use
whether it is weekend or not so drop all weekday_is columns.
...
We can see from the heat map that n_unique_tokens and
n_non_stop_unique_tokens are very highly correlated, so we drop
n_non_stop_unique_tokens. Also kw_max_min and kw_avg_min are very highly
correlated, kw_max_max and kw_min_min are very highly negatively
correlated, kw_avg_avg and kw_max_avg are very highly correlated, so for
this kw columns, we will choose kw_avg_min, kw_avg_max, and kw_avg_avg
for each worst, best, and average cases. It is always highly correlated
between three self_reference\_ variables so we use the average one
instead. we also drop rate_positive_words and rate_negative words since
it kinds of produce the same information as global ones (it also cause
high correlation with other predictors according to the heatmap above).
Moreover, we only consider avg_positive_polarity and
avg_negative_polarity since average can represent well. Lastly, we use
title_subjectivity and title_sentiment_polarity instead of absolute ones
so that we can see whether it is negatively correlated or positively
correlated with our response variable well. I think also num_hrefs and
num_self_hrefs are kind of saying the same thing so we drop
num_self_hrefs.
...
there is a clear outlier in p3, which is n_unique_tokens, what I will do
here is to drop that observation.
...
We wanna split it 60/40 There are 39643 observations in total, so we
will use 60% of it (23786) for training, 40% of it (15857) for
validation.
...
The two selection gives us different final models, but we choose the
stepwise one since it has lower AIC and the model is 'shorter'.
...
our null hypothesis here those two betas are zero, so we fail to reject
the null hypothesis which means we can drop those two variables.
...
## THE EXPERIMENT SEEMS LIKE A SUCCESS!!!

### THE RSQUARED VALUE IS BIGGER THAN ALL OF OUR OTHER MODELS

**LETS TRY DROPPING SOME PREDICTORS**

TO Drop predictors lets make a plan

### STEP 1: F TEST

Let's do a reduced model full model F Test for all interaction Terms in
this model

```{r}
anova(fit.reduce,Crazy_model)
```

Since p-value is close to 0 that means we have statically signifcant
evidence to reject H_0 at alpha = 0.05

THIS IMPLIES AT LEAST ONE OF THE INTERACTION TERMS IS NON ZERO, we will
have to do a individual check for dropping interaction terms

### STEP 2 Indivudual F Tests?

ALL THE INTERACTIONS ARE AS FOLLOWS



In the **`lm`** formula you provided for your **`Crazy_model`**, the
interaction terms are those that involve a colon (**`:`**) or a
combination of colons and plus signs (**`+`**). These terms represent
interactions between two or more variables. In your model, the
interaction terms are:

1.  **`self_reference_avg_sharess:average_token_length`**

    -   Interaction between **`self_reference_avg_sharess`** and
        **`average_token_length`**.

2.  **`self_reference_avg_sharess:avg_negative_polarity`**

    -   Interaction between **`self_reference_avg_sharess`** and
        **`avg_negative_polarity`**.

3.  **`data_channel:self_reference_avg_sharess`**

    -   Interaction between **`data_channel`** and
        **`self_reference_avg_sharess`**.

4.  **`average_token_length:global_subjectivity`**

    -   Interaction between **`average_token_length`** and
        **`global_subjectivity`**.

5.  **`data_channel:kw_avg_avg`**

    -   Interaction between **`data_channel`** and **`kw_avg_avg`**.

6.  **`is_weekend:global_subjectivity`**

    -   Interaction between **`is_weekend`** and
        **`global_subjectivity`**.

7.  **`self_reference_avg_sharess:kw_avg_avg`**

    -   Interaction between **`self_reference_avg_sharess`** and
        **`kw_avg_avg`**.

8.  **`avg_negative_polarity:average_token_length`**

    -   Interaction between **`avg_negative_polarity`** and
        **`average_token_length`**.

9.  **`self_reference_avg_sharess:is_weekend`**

    -   Interaction between **`self_reference_avg_sharess`** and
        **`is_weekend`**.

10. **`self_reference_avg_sharess:avg_negative_polarity:average_token_length`**

    -   Three-way interaction between **`self_reference_avg_sharess`**,
        **`avg_negative_polarity`**, and **`average_token_length`**.


** LETS DROP THE TERMS WHICH FAIL THE T TEST AND DO REDUCED MODEL FULL MODEL F TESTS TO TEST WHETHER WE CAN REALLY DROP THEM BUT INSTEAD OF ONLY USING R^2 ADJUST and R^2 PLUS SSE, LETS FACTOR IN AIC,BIC,PRESS,MALLAS CP AND Other things to a certain extent after doing every f test. 
...
Mallows_CP(Crazy_model,Crazy_model)
calculate_model_metrics(Crazy_model,Crazy_model)


```
THIS FUNCTION IS VERIFIED FOR NOW 


# STEP 3 START REMOVING INTERACTIONS TO TEST
WE ARE ONLY REMOVING INTERACTION TERMS WHICH HAVE A PVALUE FROM T TEST HIGHER THAN 0.05 which gives us two cases
### CASE 1
FIRST INTERACTION TO GO IS  data_channel:self_reference_avg_sharess (POTENTIAL TO REMOVE 5 - 6 predictors)


```{r}
reduced_model_1 <- lm(formula = shares ~ data_channel + self_reference_avg_sharess + 
    kw_avg_avg + num_hrefs + avg_negative_polarity + is_weekend + 
    average_token_length + global_subjectivity + self_reference_avg_sharess:average_token_length + 
...
calculate_model_metrics(Crazy_model,reduced_model_1)
anova(Crazy_model,reduced_model_1)
```
WE CAN"T DROP THIS, SO THESE INTERACTION TERMS ARE SAFE data_channel:self_reference_avg_sharess

NEXT ONE TO DROP AND TEST 
...
```{r}
reduced_model_2 <- lm(formula = shares ~ data_channel + self_reference_avg_sharess + 
    kw_avg_avg + num_hrefs + avg_negative_polarity + is_weekend + 
    average_token_length + global_subjectivity + self_reference_avg_sharess:average_token_length + 
    self_reference_avg_sharess:avg_negative_polarity + data_channel:self_reference_avg_sharess + 
...
calculate_model_metrics(Crazy_model,reduced_model_2 )

anova(Crazy_model,reduced_model_2 )

```
We decide to drop this because Mallows CP is close to p', BIC decreases, AIC increase by 0.6 but that is fine, R^2 and ADJUSTED R^2 don't decrease by a lot. Most importantly we can drop it because of t tests and Full Model Reduced Model F Tests we just conducted.

The Conclusion is, this interaction term does not contribute to our linear relationship significantly, so we can drop it.


```{r}
FINAL_INTERACTION_MODEL <- reduced_model_2 
```
...
#INTERACTING WITHOUT STEP FROM EVERYTHING

```{r}
#df = vcov(fit.reduce)
```

This suggest that avg_negative_polarity and global_subjectivity have a
high covariance value of approximately 212,367.91
...
data_channeldata_channel_is_entertainment has the highest covariance
with avg_negative_polarity (approximately 25,144.64).
data_channeldata_channel_is_lifestyle shows the highest covariance with
global_subjectivity (approximately 37,021.96).
data_channeldata_channel_is_socmed has the highest covariance with
global_subjectivity (approximately 20,005.69).
data_channeldata_channel_is_tech exhibits the highest covariance with
global_subjectivity (approximately 30,981.30).
data_channeldata_channel_is_world has the highest covariance with
global_subjectivity (approximately 30,558.54). data_channelnot specified
shows the highest covariance with global_subjectivity (approximately
44,491.91).
...
Here we can conclude that lm reduced_model_1 is the one we will be using
as our interaction model.
...
```
# TESTING THE TWO DIFFERENT INTERACTION MODELS WE FOUND
```{r}
calculate_model_metrics(Crazy_model,fit.interaction )

print("********************************")
calculate_model_metrics(Crazy_model,fit.reduce )
...
FINAL INTERACTION MODEL IS FINAL_INTERACTION_MODEL
and FINAL MAIN EFFECT MODEL IS fit.reduce

```{r}
FINAL_MAIN_EFFECT_MODEL <- fit.reduce
summary(FINAL_MAIN_EFFECT_MODEL)
```

```{r}
summary(FINAL_INTERACTION_MODEL)
```
NOW LET"S CALCUALTE THE METRICS FOR THESE 2 models

```{r}
cat("C_p for Interaction: ",Mallows_CP(fit.everything,FINAL_INTERACTION_MODEL),"\n")
cat("C_p for Main Effect ",Mallows_CP(fit.everything,FINAL_MAIN_EFFECT_MODEL),"\n")
Diff_metrics <- calculate_model_metrics(FINAL_MAIN_EFFECT_MODEL,FINAL_INTERACTION_MODEL) # 2nd -1st
Diff_metrics
```
